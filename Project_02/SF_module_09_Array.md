...
---
[Сайт по NumPy](https://pyprog.pro/short_guide.html)
...
### ***np.random.uniform(low=0.0, high=1.0, size=None)*** low - нижняя граница рандома, high - верхняя граница рандома, size - картеж с размерность. массива.

### ***np.random.randint(low, high=None, size=None, dtype=int)*** - Функцию randint нельзя запустить совсем без параметров, необходимо указать хотя бы одно число.
...
### ***np.random.choice(a, size=None, replace=True)*** послучаем случайный набор из массива
